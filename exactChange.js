...
function checkCashRegister(price, cash, cid) {
  ...
  const sumDrawer = () => {
    let drawer = 0;
    cid.forEach(tier => drawer += tier[1]);
    return drawer.toFixed(2);
  };

  const calculateChange = () => {
    console.log('need to calculate change...');
  };

  if (sumDrawer(cid) - changeDue < 0) {
    return "Insufficient Funds";
  } else if (sumDrawer(cid) - changeDue === 0) {
    return "Closed";
  } else {
